...
var count = humanOfficers.count



// Q1: What percentage of humans are female?
var percent = Float((humanOfficers.filter {femaleOfficers.contains($0)}).count) / Float(count)
print(percent)


// Q2: How many first officers are human?
var humanFirst = firstOfficers.keys.filter {humanOfficers.contains($0)}
print(humanFirst.count)



// Q3: Get a list of females sorted by name
var sorted = femaleOfficers.sorted(by: <)
print(sorted)
...
let firstVulcans : [Officer] = (firstOfficers.filter {vulcanOfficers.contains($0.key)}).map {Officer(name: $0.key,ship: $0.value)}
print(firstVulcans.map{$0.name})


// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let short = (firstVulcans.filter {$0.name.count < 5}).count
print(short)
